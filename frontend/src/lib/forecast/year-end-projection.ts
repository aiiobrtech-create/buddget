...
/** Meses com realizado informado contam como "fechados". */
...
export function scaleProjection(projection: YearEndProjection, factor: number): YearEndProjection {
  if (factor === 1) return projection
  return {
    ...projection,
    actualYtd: projection.actualYtd * factor,
    budgetYtd: projection.budgetYtd * factor,
    budgetAnnual: projection.budgetAnnual * factor,
    budgetRemaining: projection.budgetRemaining * factor,
    projectedRemaining: projection.projectedRemaining * factor,
    forecastYearEnd: projection.forecastYearEnd * factor,
  }
}
